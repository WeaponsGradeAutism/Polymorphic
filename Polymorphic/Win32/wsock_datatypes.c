...
int connection_array_set(connection_array *vector, uint32_t index, CONNECTION *connection) 
{
	...
	if (connection == NULL || vector->connections.data[index] == NULL || index >= vector->connections.size || index < 0)
		return VECTOR_INVALID_ARGUMENT;
...
int service_connection_array_append(service_connection_array *vector, CONNECTION *connection)
{
	...
	vector->connections.members++;
	vector->connections.data[index] = connection;
	vector->connections.auxConnectionContainers[index] = malloc(sizeof(connection_array));
	return index;
}
...
int service_connection_array_push(service_connection_array *vector, CONNECTION *connection) {
...
int service_connection_array_push_aux(service_connection_array *vector, int index, CONNECTION *connection) {
	...
	return connection_array_push(auxconnList, connection);
}
...
int service_connection_array_delete(service_connection_array *vector, uint32_t index)
{
	...
	if (ARRAY_NOT_EMPTY == connection_array_free(vector->connections.auxConnectionContainers[index]))
		return ARRAY_NOT_EMPTY;
...
uint32_t service_connection_array_close_all_aux(service_connection_array *vector, uint32_t index) {
	connection_array *auxConnectionContainers = &vector->connections.auxConnectionContainers[index];
	for (uint32_t x = 0; x < auxConnectionContainers->connections.size; x++)
	{
		CONNECTION *connection = connection_array_get(auxConnectionContainers, x);

		if (connection == NULL)
			continue;

		closeConnection(connection);
		connection_array_delete(auxConnectionContainers, x);
	}
...
void service_connection_array_free(service_connection_array *vector) {
