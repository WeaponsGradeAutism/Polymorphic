...
///<summary> Initializes the memory allocation array at the default size. </summary>
void memory_allocation_array_init(memory_allocation_array *vector)
{
	memory_allocation_array_init_capacity(vector, VECTOR_INITIAL_CAPACITY);
}

///<summary> Initializes the memory allocation array at the given size. </summary>
void memory_allocation_array_init_capacity(memory_allocation_array *vector, int size)
{
	vector->messages = malloc(sizeof(memory_allocation*) * size);
	vector->size = size;
	vector->count = 0;
	vector->nextIndex = 0;
	...
	for (int x = 0; x < size; ++x)
	{
		vector->messages[x] = malloc(sizeof(memory_allocation));
		vector->messages[x]->index = -1;
	}
}

///<summary> Expands the memory allocation array by the given amount. </summary>
void memory_allocation_array_extend(memory_allocation_array *vector, unsigned int amount)
{
	int oldsize = vector->size;
	vector->size += amount;
	vector->messages = realloc(vector, sizeof(memory_allocation*) * vector->size);
...
///<summary> Finds an unused memory allocation in the array, initializes it, and returns a pointer to it. </summary>
memory_allocation* memory_allocation_array_allocate(memory_allocation_array *vector)
{
	...
	// no open spaces found, expand the buffer for some more messages
	memory_allocation_array_extend(vector, 10);
	return memory_allocation_array_allocate(vector);
}

///<summary> Marks the specified memory allocation as freed. </summary>
...
///<summary> Frees the memory allocation array object. </summary>
